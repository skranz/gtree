...
game %>%
  ...
  game.eq.tables()
```
We get one equilibrium that is sequentially rational. Interesting this is a mixed strategy equilibrium, in which the responder is equally likely to accept or rejet a zero offer.


Another approach is to modify the game to manually add trembles in order to select equilibria that will usually coincide with trembling-hand perfect equilbria. The call to function `game_add_tremble` in the following code changes the game structure such that with probability 0.01%, the proposer trembles and picks a random offer.
```{r}
game %>%
...
Looking at the equilibria, we see that only the two intuitive pure strategy equilibria where the proposer offers 0 or 1 survive.  

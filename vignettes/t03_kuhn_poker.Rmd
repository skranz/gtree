...
game %>%
  game_fix_action_preferences(cb1="check", util.add=1000) %>%
  game_gambit_solve(mixed=TRUE) %>%
  eq_cond_expected_outcomes("pl1_card","cb1") %>%
  select(payoff_1,payoff_2,pl1_card, cb1, fc1, cb2,fc2, is.eqo)  
  
game %>%
  game_fix_action_preferences(cb1=~ifelse(pl1_card<=2,"check","bet"), util.add=10) %>%
  game_gambit_solve(mixed=TRUE) %>%
  eq_cond_expected_outcomes("pl1_card","cb1") %>%
  select(payoff_1,payoff_2, cb1, fc1, cb2,fc2, is.eqo)  

game %>%
  game_fix_action_preferences(cb1=~ifelse(pl1_card<=2,"check","bet"), util.add=10) %>%
  game_gambit_solve(mixed=TRUE) %>%
  eq_cond_expected_outcomes("pl1_card","pl2_card", "cb1") %>%
  select(payoff_1,payoff_2,pl1_card, pl2_card, cb1, fc1, cb2,fc2, is.eqo)  


game %>%  eq_expected_outcomes()  %>%
  select(payoff_1,payoff_2, cb1, fc1, cb2,fc2)  
